...
class PrinterUVLED:
    def __init__(self, config):
        ...
        if static_value is not None:
            ...
            gcode.register_command('LED_ON',self.cmd_LED_ON)
            gcode.register_command('LED_OFF',self.cmd_LED_OFF)
    ...
    def cmd_LED_ON(self, gcmd):
        value = self.pwm_value
        value /= self.scale
        cycle_time = gcmd.get_float('CYCLE_TIME', self.default_cycle_time,
                                    above=0., maxval=MAX_SCHEDULE_TIME)
        if not self.is_pwm and value not in [0., 1.]:
